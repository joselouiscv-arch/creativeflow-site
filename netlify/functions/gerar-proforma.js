...
exports.handler = async (event) => {
  ...
  const dataEmissaoObj = new Date();
  const dataValidadeObj = new Date();
  dataValidadeObj.setDate(dataValidadeObj.getDate() + 15);
  ...
  const css = `
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .page { background: #fff; width: 794px; min-height: 1123px; padding: 28px 34px 20px; color: #111; display: flex; flex-direction: column; position: relative; page-break-after: always; }
    ...
    .fatura-word { font-family: 'Barlow Condensed', sans-serif; font-size: 35px; font-weight: 900; text-transform: uppercase; line-height: 1; letter-spacing: 3px; }
    .date-boxes { display: flex; gap: 5px; margin-top: 9px; }
  ...
  const footerCompleto = `
    <div class="bottom-section">
      <div class="payment-terms">
        ...
        <div class="terms-text">Esta pro-forma é um documento provisório emitido para efeitos de orçamento e não constitui obrigação de pagamento imediato. Os valores indicados são válidos por 15 dias a partir da data de emissão. Para confirmar a encomenda ou serviço, contacte-nos pelos meios indicados neste documento.</div>
      </div>
        ...
        <div class="header">
          ...
          <div class="company-info">
            ...
            <div class="company-details">
              ${e.morada ? `<b>Endereço:</b><span>${esc(e.morada)}</span>` : ''}
              ${e.nif ? `<b>NºNIF:</b><span>${esc(e.nif)}</span>` : ''}
              ${e.telefone ? `<b>Contato:</b><span>${esc(e.telefone)}</span>` : ''}
              ${e.email ? `<b>E-Mail:</b><span>${esc(e.email)}</span>` : ''}
              ${e.conta ? `<b>Nº Conta:</b><span>${esc(e.conta)}</span>` : ''}
            </div>
          </div>
        </div>
        <hr class="divider">
        <div class="client-row">
          <div class="client-info">
            ...
            <div class="client-details">
              ${c.morada ? `<b>Endereço:</b><span>${esc(c.morada)}</span>` : ''}
              ${c.nif ? `<b>Contribuinte:</b><span>${esc(c.nif)}</span>` : ''}
              ${c.telefone ? `<b>Contato:</b><span>${esc(c.telefone)}</span>` : ''}
              ${c.email ? `<b>E-Mail:</b><span>${esc(c.email)}</span>` : ''}
            </div>
          </div>
          <div class="right-block">
            <div class="fatura-word">Fatura Pró Forma</div>
            <div class="date-boxes">
              <div class="date-box"><div class="date-label">Data de Emissão</div><div class="date-value">${formatDate(dataEmissaoObj)}</div></div>
              <div class="date-box"><div class="date-label">Válido por 15 Dias</div><div class="date-value">${formatDate(dataValidadeObj)}</div></div>
            </div>
          </div>
        </div>
        ${tabela(rows)}
  ...
  const html = `<!DOCTYPE html><html lang="pt"><head><meta charset="UTF-8"><title>Fatura Pró Forma</title><style>${css}</style></head><body>${paginas}</body></html>`;
  ...
  try {
    ...
    return {
      ...
      body: JSON.stringify({ pdf: Buffer.from(pdf).toString('base64'), filename: 'proforma.pdf' }),
    };
  } catch (err) {
